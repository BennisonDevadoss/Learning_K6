      ...
      for (let item of messages) {
        ...
        simpleParser(idHeader + all.body, (err, mail) => {
          ...
          if (sub == "Reset your ChiefNET password") {
            ...
            let resetPasswordLink = linkArray[0].replace(
              "https://dev.chiefnet.yavar.in/resetpassword?password_token=",
              ""
            ); /* Change this line and the below line */
...
setTimeout(function () {
  readLastLines
    .read("./link.txt", 1) // have to make changes
    .then((lines) => (link = lines));
}, 20000);

setTimeout(function () {
  fs.truncate("./link.txt", 0, function () {
    console.log("done");
  }); /* To delete the file contents */
}, 22000);
